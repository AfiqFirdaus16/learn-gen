...
router.post('/', async (req, res) => {
  try {
    ...
    if (!validRoles.includes(role)) return res.status(400).json({ error: 'Peran pengguna tidak valid' });
    if (password.length < 6) return res.status(400).json({ error: 'Password minimal 6 karakter' });

    const user = await prisma.admin.create({
      data: { nama: nama.trim(), email: email.trim().toLowerCase(), password: await bcrypt.hash(password, 10), role },
      select: publicUserSelect,
    });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (id === req.admin.id && role !== 'admin') return res.status(400).json({ error: 'Admin tidak dapat menghapus perannya sendiri' });

    const data = { nama: nama.trim(), email: email.trim().toLowerCase(), role };
    if (password) data.password = await bcrypt.hash(password, 10);
    const user = await prisma.admin.update({ where: { id }, data, select: publicUserSelect });
